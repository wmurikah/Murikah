...
| 13  | `users/13_job_title_mappings.sql`                                                | **Two additive catalogue tables: `job_title_access_role_mappings` and `job_title_workflow_role_mappings`.** Defaults offered when a job title is set; nothing in the permission or approval path reads them, and both start empty                                                                                              | Yes                                  | Yes, drop the two tables                                              | Its own three `SELECT`s: (a) two rows, (c) NO rows        |
