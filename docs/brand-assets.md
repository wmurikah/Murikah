...
| `docs/images/murikah_6.png` | Horizontal lockup (white M + brass accent + divider + MURIKAH). The current file is a Canva export on a **light-grey** background (opaque, no alpha channel); an earlier revision of the same lockup was on **black**. | Header logo is `public/images/murikah-logo-transparent.png`: a genuinely transparent white-and-brass lockup keyed from the white-on-black revision (kept at `public/images/murikah-logo-dark.png`, alt "Murikah"). The light-grey export is not used directly, since on the slate header its white wordmark would vanish and its light box would show. |

`murikah_1.png` is the website theme reference. It is not a logo asset.

The header logo (`public/images/murikah-logo-transparent.png`) was produced by
keying the flat background out of the white-on-black lockup to add a real alpha
channel, so it sits directly on the slate header (`--color-header-bg`, `#1E2A30`)
with no background block and no blend trick. The provided `murikah_6.png` export
has a light-grey background rather than transparency, so it could not be used
as-is on a dark bar.
