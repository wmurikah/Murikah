...
test('the shared overlay script has one lifecycle and restores the actual trigger', () => {
  const overlay = readFileSync('src/components/cms/CmsOverlayScript.astro', 'utf8');
  assert.doesNotMatch(overlay, /mayClose/);
  assert.equal((overlay.match(/'close',/g) ?? []).length, 1);
  assert.equal((overlay.match(/'cancel',/g) ?? []).length, 1);
  assert.equal((overlay.match(/open\(dialog, trigger\)/g) ?? []).length, 1);
  assert.equal((overlay.match(/close\(dialog, true\)/g) ?? []).length, 1);
});
