...
Phase 8 adds the deterministic qualifying-completion subsystem without changing the authority established by Phases 1 through 7. Migration `0014_virtual_internship_phase8_completion.sql` extends the top-level internship and learner-membership lifecycle to `active`, `stopped` and `completed`, adds the immutable `scenario_completion_policies` and `completion_records` authorities, and adds the durable `internship_completed` lifecycle event. The migration does not edit `0006_virtual_internship_phase1.sql`. Because the Phase 1 parent tables are referenced by later migrations, the lifecycle constraint extension is additive: the original always-null Phase 1 completion guard is retained as `phase1_completed_at_guard`, while the new authoritative `completed_at` is server-written only at successful Phase 8 finalization. Existing records, foreign keys, ownership history, indexes and the one-active-qualifying rule are preserved.

The authored completion-policy schema is `tutor/virtual-internship/schema/v1/completion-policy.schema.json`. A scenario version may carry a sibling `completion.json`; `virtual_internship.validator` validates it against the schema and the scenario's exact task and competency references, and runtime bootstrap installs its canonical SHA-256 hash into `scenario_completion_policies` for the exact pinned scenario version. The policy supports explicit required task IDs, ordinary required review types, versioned competency/evidence requirements, capstone task IDs, a required-final-review flag and an after-capstone final-review rule. It does not duplicate the Phase 1 duration authority. Published historical scenario definitions and content hashes are not rewritten. A first completion policy cannot be attached after any internship has already pinned that scenario version; such versions remain fail-closed rather than receiving retroactive requirements. A pinned version with no valid installed completion policy fails closed with `completion_policy_unavailable`; the Phase 1 foundation version and the existing demo packs therefore do not become qualifying-completable merely because Phase 8 exists.

The authoritative Worker service is `tutor/cloudflare/src/virtual_internship_phase8.ts`. Its initial gate evaluator/ruleset version is `phase8-completion-evaluator-v1`. `evaluateCompletionEligibility` is a read-only deterministic evaluation over persisted D1 state and server time. It performs zero model/provider calls and no R2 artifact downloads. The duration gate uses the internship's Phase 1 snapshotted `minimum_duration_days`; required work uses canonical Phase 2 task state after the Phase 5 acceptance workflow; ordinary required reviews use finalized Phase 6 review rows; competency gates consume the Phase 7 Passport aggregate plus stable underlying evidence references and ruleset versions; capstone completion uses canonical task state; and an authored after-capstone rule compares persisted capstone-completion and final-review timestamps. There is no percent-complete authority and no model prose is parsed as completion evidence. No pause/leave model existed before Phase 8, so Phase 8 retains the Phase 1 wall-clock duration contract and does not infer qualifying time from sessions or logins.

Eligibility and finalization are separate. Learners read owner-scoped status through `GET /api/murikah/virtual-internship/completion`. An explicit `POST /api/murikah/virtual-internship/completion` supplies only the owned internship ID plus an idempotency request ID. The Worker immediately re-evaluates every gate before writing. Successful finalization uses one D1 batch to create exactly one immutable completion record, move the internship and learner membership to `completed`, set the server-generated UTC `completed_at`, and record `internship_completed`. Repeated or concurrent completion attempts converge on the one unique completion record. The stop path is separately guarded so a stop/completion race cannot write both terminal histories. Stopped internships cannot complete and completed internships cannot return to active through learner actions.

Each `completion_records` row stores the exact scenario pack/version, completion-policy schema/hash, required duration, evaluator version, canonical structured gate snapshot, deterministic snapshot SHA-256, request ID, Passport aggregation-ruleset versions, evidence-ruleset versions and stable supporting evidence IDs. The record is immutable in normal operation and remains the future Phase 9 source of truth. Later changes to policies, competency definitions or Passport aggregation do not silently recalculate an already finalized historical completion.

Qualifying protection is server-side. A completion requires the lifecycle row to remain `qualifying = 1` and `mode = standard`, and the pinned scenario manifest must itself be authored as `qualifying`. Demo mode, test mode, non-qualifying rows and current demo scenario manifests cannot issue qualifying completion even if simulated gates otherwise appear satisfied. There are no `skip90days`, `forceComplete` or similar production bypasses. Accelerated time is confined to deterministic tests.

After completion, Phase 2 scenario mutations, Phase 3 AI-invocation/assistance writes, Phase 4 workplace mutations, Phase 5 artifact mutations and Phase 6 assessment/review mutations remain blocked by active-status guards; late Phase 5 task-completion activity and Phase 6 assessment completion/failure also re-check the active terminal state at the D1 write boundary. The shared Phase 4 workspace stays readable and now renders a professional Completion requirements area with separate duration, required-work, review, competency-evidence, capstone and final-review states. The final action appears only after the latest server evaluation is eligible and does not optimistically render completion. Completed work, reviews and the Competency Passport remain reviewable in read-only form, with the Murikah simulation disclosure retained.
...
Future phases must treat immutable Phase 8 `completion_records` as the authority for qualifying completion. Phase 9 reports and letters must consume the stored completion snapshot and must not independently re-evaluate or override Phase 8 gates. Preserve the exact completion-policy hash, evaluator version, Passport aggregation/evidence ruleset versions and supporting evidence references recorded at finalization.

Future changes must preserve demo/test non-qualification, authenticated learner ownership, server-authoritative UTC `completed_at`, completed/stopped terminal semantics and the one-active-qualifying rule. Do not silently rewrite or invalidate historical completion when policy, competency or aggregation rules change. Any future administrative correction requires an explicit audit-preserving supersession/revocation design outside normal learner actions. Any future institution endorsement remains a separate verified institution workflow and must not be implied by Murikah completion.
