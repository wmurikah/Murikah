...
OLD_SIGNATURE = '''async def guest_chat(\n    body: GuestChatRequest,\n    response: Response,\n    mt_guest: str | None = Cookie(default=None, alias=_COOKIE_NAME),\n) -> dict:\n'''
NEW_SIGNATURE = '''async def guest_chat(\n    body: GuestChatRequest,\n    response: Response,\n    mt_guest: str | None = Cookie(default=None, alias=_COOKIE_NAME),\n) -> Response:\n'''

# This is the exact block produced earlier in the Docker overlay by
# enhance_guest_diagram.py. Match only the completion section so this patch is
# not coupled to the quota/return lines that follow it.
...
def patch_backend(root: Path) -> None:
    ...
    text = replace_once(text, OLD_SIGNATURE, NEW_SIGNATURE, "guest-chat signature")
    text = replace_once(text, OLD_BACKEND, NEW_BACKEND, "post-diagram guest completion")
    text = replace_once(text, OLD_RETURN, NEW_RETURN, "guest streamed return")
