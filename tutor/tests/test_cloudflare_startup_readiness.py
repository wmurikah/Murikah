...
class CloudflareStartupReadinessTests(unittest.TestCase):
    ...
    def test_source_deploy_uses_live_revision_gate_without_application_recycling(self):
        ...
        self.assertIn("Asynchronous rollout lag is not a build failure", text)
        self.assertIn("sawExpectedRevision", text)
        self.assertIn("hadStatus", text)
    ...
    def test_worker_refuses_stale_container_revision(self):
        ...
        self.assertIn("Replacing stale Murikah Tutor image revision", text)
        self.assertIn("nextRevisionRestartAt", text)
