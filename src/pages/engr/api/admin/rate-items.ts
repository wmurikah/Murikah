...
/** Rate item write endpoint: add an item to a rate card. Gate: rates.manage. */
import type { APIRoute } from 'astro';
import { getEngrEnv } from '@engr/env';
import { getDb } from '@engr/db';
import { addRateItem, type RateItemInput } from '@engr/repos/adminRateCards';
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const result = await addRateItem(db, ctx, input);

  if (result.ok) {
    return wantsJson(request)
      ? jsonResponse({ ok: true, id: result.id }, 201)
      : seeOther(`/admin/rate-cards/${rateCardId || 'new'}?saved=1`);
  }
  ...
  return seeOther(`/admin/rate-cards/${rateCardId || 'new'}?error=${result.code}`);
};
