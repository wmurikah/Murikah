...
export const ACTING_COOKIE = 'engr_acting_org';
const MAX_AGE_SECONDS = 12 * 60 * 60; // 12 hours, matching the session
...
/** Read and verify the acting-org cookie, returning the organisation id or null. */
export async function readActingOrg(request: Request, secret: string): Promise<string | null> {
  const token = readCookie(request, ACTING_COOKIE);
...
export async function createActingCookie(
  ...
  const token = await new SignJWT({ act: orgId })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuedAt()
    .setExpirationTime(`${MAX_AGE_SECONDS}s`)
    .sign(keyFromSecret(secret));
  const secureAttr = secure ? '; Secure' : '';
  return `${ACTING_COOKIE}=${token}; HttpOnly${secureAttr}; SameSite=Lax; Path=/; Max-Age=${MAX_AGE_SECONDS}`;
}
